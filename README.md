...
После выполнения предсказания worker отправляет результат в приложение по HTTP, а FastAPI выполняет бизнес-логику сохранения результата и списания средств.
...
Если средств недостаточно для всех корректных строк, пакет целиком отклоняется с `HTTP 402`.
...
- `pytest`;
- `requests`;
- pytest fixtures для подготовки независимых сценариев.
...
Перед тестированием система должна быть запущена:
...
Каждый тест является отдельным сценарием. Если один тест завершается с ошибкой, pytest продолжает выполнение остальных тестов.

Для подготовки данных используются fixtures. Например:

- `registered_user` — создаёт нового пользователя;
- `funded_user` — создаёт нового пользователя и пополняет его баланс на `10.00`;
- `batch_funded_user` — создаёт нового пользователя и пополняет баланс на `20.00`;
- `completed_prediction` — создаёт подготовленного пользователя, отправляет ML-задачу и ожидает её завершения.

Для каждого теста создаётся новый пользователь с уникальным логином, поэтому тесты не зависят от результатов предыдущих тестов.

Тесты проверяют:

- доступность API;
- регистрацию;
- авторизацию и повторную авторизацию;
- ошибку при неверном пароле;
- повторную регистрацию;
- защиту endpoints от неавторизованного доступа;
- начальный баланс;
- пополнение баланса;
- запрет отрицательного пополнения;
- отсутствие списания при ошибочном ML-запросе;
- создание и асинхронное выполнение ML-задачи;
- списание после успешного предсказания;
- запрет ML-запроса при недостаточном балансе;
- историю транзакций;
- историю предсказаний;
- пакетную обработку частично некорректных данных;
- списание только за корректно обработанные строки;
...
Подробное описание тестирования приведено в [`TESTING.md`](TESTING.md).
